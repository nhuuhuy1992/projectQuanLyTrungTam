...
function showDSND(DSND:Array<NguoiDung>){
	let data:string = "";
...
$("#btnThemNguoiDung").click( () =>{
	let HoTen     = $("#HoTenND").val();
	let TaiKhoan     = $("#TaiKhoanND").val();
	let SoDT     = $("#SoDTND").val();
	let maND     = $("#maND").val();
	let MatKhauND     = $("#MatKhauND").val();
	let Email     = $("#EmailND").val();
...
$("#timND").keyup(function(){
	...
	if(key === "" || key === " " || DSNDCanTimKiem.DSND.length == 0){
		...
		let trNDTimKiem = $(".trNguoiDungTimKiem");
		for(let i:number = 0; i < trNDTimKiem.length; i++){
			trNDTimKiem[i].addEventListener("click", function(){
				$("#tableTimKiemNguoiDung").removeClass("active");
				let tkCanTim = $(`#${$(this).data("choose")}`);
				$(tkCanTim).addClass("choose");
				window.scroll({
